...
// R093's integration guard. Every leg gets a fresh Chrome process, profile,
...
if (process.argv.length !== 3) {
  throw new Error("usage: node tests/run-r093-wanix-browser-pairs.mjs RESULTS_DIR");
}
...
const lockPath = resolve(tmpdir(), "rv64-r093-wanix-browser.lock");
...
for (let attempt = 0; attempt < 3; attempt++) {
  try {
    ...
    if (Number.isSafeInteger(owner) && owner > 0 && processIsAlive(owner)) {
      throw new Error(`another R093 WANIX benchmark is running (pid ${owner})`);
    }
    rmSync(lockPath, { recursive: true, force: true });
    if (attempt === 2) throw new Error("could not acquire the R093 WANIX lock");
  }
}
let lockHeld = true;
...
const pages = {
  ...
  candidate: {
    url: "http://127.0.0.1:8765/examples/" +
      "v86-rv64-three-way-r093-product-89d3aa449a28d135.html",
    path: resolve(root,
...
const archives = {
  ...
  candidate: {
    path: resolve(root, "target/bench/r093-integrated-scalar-t0/browser-assets/" +
      "rv64-jit-r093-product-89d3aa449a28d135.tgz"),
    url: "http://127.0.0.1:8765/rv64/rv64-jit-r093-product-89d3aa449a28d135.tgz",
    sha256: "89d3aa449a28d135aaca3af8025fcb97f5b817d335be6eee2bff374859f35669",
    wasmSha256: "b40bb5f3f55b04f7b6bea424011a7963313b363b3a8b0f8cbff161319bd3451e",
  },
};
...
const protocol = {
  schema: 1,
  experiment: "R093 immutable-artifact WANIX browser guard",
  ...
  toolManifest: {
    ...
    analyzer: digestFile(resolve(root, "tests/analyze-r093-wanix-browser-pairs.mjs")),
...
try {
  for (const { pair, sides } of order) {
    for (const side of sides) {
      process.stderr.write(`R093 WANIX pair ${pair}/${pairs}: ${side}\n`);
...
process.stderr.write(`R093 fixed WANIX sample complete: ${resultsDirectory}\n`);
