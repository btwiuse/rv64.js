...
import {
  evaluateR079SourceRestoration,
  R079_CANDIDATE_WASM_SHA256,
  R079_CONTROL_WASM_SHA256,
  R079_LOADER_SHA256,
  R079_REPS,
  R079_ROWS,
} from "./r079-source-restoration-lib.mjs";
...
function synthetic() {
  const trials = [];
  for (const row of R079_ROWS) {
    for (let rep = 1; rep <= R079_REPS; rep++) {
  ...
  return {
    ...
    configuration: {
      ...
      candidateConfig: {},
      wasmBySide: {
        control: { path: "/control.wasm", sha256: R079_CONTROL_WASM_SHA256 },
        candidate: { path: "/candidate.wasm", sha256: R079_CANDIDATE_WASM_SHA256 },
      },
...
const accepted = evaluateR079SourceRestoration(synthetic(), "synthetic");
assert.equal(accepted.gatePassed, true, accepted.problems.join("; "));

function reject(name, mutate, needle) {
  const report = synthetic();
  mutate(report);
  const result = evaluateR079SourceRestoration(report, "synthetic");
...
console.log("r079 source restoration selftest: PASS");
