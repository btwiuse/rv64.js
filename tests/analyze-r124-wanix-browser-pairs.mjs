...
if (!process.argv[2] || !outputArgument) {
  throw new Error(
    "usage: node tests/analyze-r124-wanix-browser-pairs.mjs RESULTS_DIR --output=REPORT",
  );
}
const protocol = JSON.parse(readFileSync(join(directory, "protocol.json"), "utf8"));
assert.equal(protocol.schema, 1);
assert.equal(protocol.experiment, "R124 fixed-bank immutable-artifact WANIX browser guard");
...
const report = {
  ...
  method: {
    ...
    comparison: "exact R124 control versus exact R124 fixed-register-bank candidate",
...
if (problems.length) throw new Error(`R124_WANIX_BROWSER_GATE_FAIL: ${problems.join("; ")}`);
console.log("R124_WANIX_BROWSER_GATE_PASS");
