...
import { evaluateR080SourceRestoration } from "./r080-residual-cleanup-lib.mjs";

const args = process.argv.slice(2);
const outputIndex = args.indexOf("--output");
if (args.length !== 3 || outputIndex < 0 || outputIndex === args.length - 1) {
  throw new Error(
    "usage: node tests/vs-v86/r080-residual-cleanup-gate.mjs " +
...
const result = evaluateR080SourceRestoration(JSON.parse(bytes), sha256);
...
if (result.gatePassed) {
  console.log("R080_SOURCE_RESTORATION_GATE_PASS");
} else {
  console.error(`R080_SOURCE_RESTORATION_GATE_FAIL: ${result.problems.join("; ")}`);
  process.exitCode = 1;
}
