...
import {
  ...
  runTimedBoot,
  yieldsAfterPump,
} from "./r077-browser-boot-lib.mjs";
import {
  collectR077Result,
  r077AssetManifest,
} from "./r077-browser-host.mjs";
...
await assert.rejects(
  collectR077Result({
    async send() {
      errorPolls++;
      return { result: { value: { done: true, text: '{"error":"worker failed"}' } } };
    },
  }, { attempts: 2, delayFn: async () => {} }),
  /worker failed/,
);
assert.equal(errorPolls, 1, "completed Worker errors must propagate without polling to timeout");

const worker = readFileSync(join(root, "tests/jit-modern-boot-production-browser-worker.mjs"), "utf8");
const sourceOrder = [
  "fetchBytes(\"/rv64.wasm\")",
  "await RV64.create(wasm)",
  "vm.ex.jit_set_page_policy(1)",
  "vm.bootVirtLinuxDirect({",
  "const productionLifecycle = vm.prepareProductionSystemT0()",
  "if (variant === \"control\")",
...
assert.match(worker, /vm\.ex\.jit_set_static_t0_sampled_backoff\(0\)/);
assert.match(worker, /vm\.ex\.jit_set_static_t0_sampled\(0\)/);
assert.doesNotMatch(worker, /rowKey|benchmark|guest PC|binary hash/);
assert.match(worker, /productionHelper: productionLifecycle\.production/);
assert.match(worker, /registeredModules: productionLifecycle\.registeredModules/);
assert.match(worker, /systemEnabled: false/);
assert.match(worker, /sampledEnabled: production/);
assert.match(worker, /sampledBackoffEnabled: production/);
...
assert.match(authoritative, /vm\.prepareProductionSystemT0\(\)/);

const manifest = r077AssetManifest();
...
console.log(
  "R077 harness selftest: scorecard timer boundary, 2M quantum, 1/5/9 yield cadence, marker, and assets enforced",
);
