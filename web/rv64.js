  ...
  jitStats() {
    ...
    return {
      generated: {
        ...
        dispatchEmptyMisses: stat(154),
        dispatchTagCollisions: stat(155),
        dispatchReverifications: stat(156),
        zeroRetireTracked: stat(157),
        zeroRetireUntracked: stat(158),
        zeroRetireProfileResets: stat(159),
        zeroRetireProfiles: stat(160),
        zeroRetireSuppressedEntries: stat(161),
        tlbFills: stat(31),
        chainHops: stat(80),
        tlbFillKinds: {
          loadHit: stat(81),
...
RV64Debug.prototype.virtioConsoleInput = function (bytes) {
  const ptr = this.ex.staging_alloc(bytes.length);
  new Uint8Array(this.ex.memory.buffer, ptr, bytes.length).set(bytes);
  ...
  #assemble() {
    ...
    if (boot.mode === "firmware") {
      ...
      this.#input = boot.virtioConsole
        ? (bytes) => this.#core.virtioConsoleInput(bytes)
