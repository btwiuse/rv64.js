...
import { evaluateR078ArtifactRegression } from "./r078-artifact-regression-lib.mjs";

const args = process.argv.slice(2);
const outputIndex = args.indexOf("--output");
if (args.length !== 3 || outputIndex < 0 || outputIndex === args.length - 1) {
  throw new Error(
    "usage: node tests/vs-v86/r078-artifact-regression-gate.mjs " +
...
const result = evaluateR078ArtifactRegression(JSON.parse(bytes), sha256);
...
if (result.gatePassed) {
  console.log("R078_ARTIFACT_REGRESSION_GATE_PASS");
} else {
  console.error(`R078_ARTIFACT_REGRESSION_GATE_FAIL: ${result.problems.join("; ")}`);
  process.exitCode = 1;
}
