...
if (!process.argv[2] || !outputArgument) {
  throw new Error(
    "usage: node tests/analyze-r085-chrome-boot-pairs.mjs RESULTS_DIR --output=REPORT",
  );
}
...
assert.equal(protocol.experiment, "R085 artifact A/B Chrome execution-only modern Boot");
...
console.log(
  `R085 Chrome Boot: ${pairedMedian.toFixed(3)}x ` +
  `CI=[${confidence95.map((value) => value.toFixed(3)).join(", ")}]`,
);
if (problems.length) throw new Error(`R085_CHROME_BOOT_GATE_FAIL: ${problems.join("; ")}`);
console.log("R085_CHROME_BOOT_GATE_PASS");
