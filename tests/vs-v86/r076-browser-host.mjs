...
export const R076_ASSETS = Object.freeze({
  page: {
    path: join(root, "tests/jit-modern-boot-browser.html"),
    route: "/jit-modern-boot-browser.html",
  },
  worker: {
    path: join(root, "tests/jit-modern-boot-browser-worker.mjs"),
  ...
  loader: {
    ...
    route: "/web/rv64.js",
    expectedSha256: "2582e18ea207ad0f5ec154e82d3fe6208faf5742f26a9d8463d8d360f168b776",
  },
  wasm: {
...
export function r076AssetManifest() {
  return Object.fromEntries(Object.entries(R076_ASSETS).map(([name, asset]) => {
...
export async function startR076Server() {
  const byRoute = new Map(Object.values(R076_ASSETS).map((asset) => [asset.route, asset]));
...
async function pageTarget(browserUrl, expected) {
  ...
  throw new Error("R076 benchmark page target did not appear");
}
...
async function collect(cdp) {
  ...
  throw new Error(`R076 browser benchmark timeout: ${lastError ?? "no result"}`);
}

export async function runR076BrowserSample({ origin, variant, chrome }) {
  if (variant !== "control" && variant !== "candidate") {
    throw new Error(`invalid R076 variant ${variant}`);
  }
  const profile = mkdtempSync(join(tmpdir(), `rv64-r076-${variant}-chrome-`));
  const url = `${origin}/jit-modern-boot-browser.html?variant=${variant}`;
