...
// docs/jit-rewrite/R074_SHORT_SAMPLE_BACKOFF_PROTOCOL.md.
...
if (!reportArg) {
  throw new Error("usage: node tests/vs-v86/r074-short-sample-backoff-gate.mjs REPORT.json");
}
...
const expectedControl = {
  ...
  SCORECARD_V2_SAMPLED_STATIC_T0_BACKOFF: "0",
};
const expectedCandidate = {
  SCORECARD_V2_STATIC_SYSTEM_T0: "0",
  SCORECARD_V2_SAMPLED_STATIC_T0: "1",
  SCORECARD_V2_SAMPLED_STATIC_T0_BACKOFF: "1",
};
const rules = {
...
for (const trial of report.trials ?? []) {
  ...
  if (
    ...
    lifecycle.sampledBackoff !== candidate ||
    lifecycle.modulesAfter !== lifecycle.modulesBefore + 1
  ) {
    problems.push(`${trial.side}/${trial.row}/rep${trial.rep}: invalid R074 lifecycle`);
  }
  ...
  const marks = BigInt(counters.staticT0ShortMarks ?? "0");
  const bypasses = BigInt(counters.staticT0ShortBypasses ?? "0");
  if (
    candidate &&
    (fast === 0n || sampled === 0n || samples === 0n || polls === 0n ||
      marks === 0n || bypasses === 0n)
  ) {
    problems.push(`${trial.side}/${trial.row}/rep${trial.rep}: incomplete R074 proof`);
  }
  if (
    !candidate &&
    (fast !== 0n || sampled !== 0n || samples !== 0n || polls !== 0n ||
    ...
    problems.push(`${trial.side}/${trial.row}/rep${trial.rep}: control executed R074 path`);
  }
  if (errors !== 0n) {
...
if (problems.length) {
  console.error(`R074_NATIVE_GATE_FAIL: ${problems.join("; ")}`);
  process.exitCode = 1;
} else {
  console.log("R074_NATIVE_GATE_PASS");
}
