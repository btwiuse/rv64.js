...
if (!reportArg) {
  throw new Error("usage: node tests/vs-v86/r077-chrome-boot-gate.mjs ANALYSIS.json");
}
...
if (report.experiment !== "R077 production-default Chrome execution-only modern Boot") {
...
if (report.assetManifest?.loader?.sha256 !==
    "d949d8641dd4048ed031c7293ddf9d7b7c911dbc89aa9fa0c29487c21687718b") {
  problems.push("production loader changed");
}
if (report.assetManifest?.wasm?.sha256 !==
    "28ceaf7bcf63b7267b25c5a9542e111d6b4f9f2b5780517ca14d89895ce10b3c") {
  problems.push("main Wasm changed");
}
...
if (!execution || execution.control?.length !== 7 || execution.production?.length !== 7 ||
...
if (!mips || mips.control?.length !== 7 || mips.production?.length !== 7 ||
...
for (const side of ["control", "production"]) {
  ...
  for (const field of ["staticFast", "sampled", "samples", "polls", "marks", "bypasses"]) {
    if (side === "production" && !(counters[field] > 0)) {
      problems.push(`production ${field} proof missing`);
    }
...
if (problems.length) {
  console.error(`R077_CHROME_BOOT_GATE_FAIL: ${problems.join("; ")}`);
  process.exitCode = 1;
} else {
  console.log("R077_CHROME_BOOT_GATE_PASS");
}
