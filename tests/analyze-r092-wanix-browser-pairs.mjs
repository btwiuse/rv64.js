...
if (!process.argv[2] || !outputArgument) {
  throw new Error(
    "usage: node tests/analyze-r092-wanix-browser-pairs.mjs RESULTS_DIR --output=REPORT",
  );
}
const protocol = JSON.parse(readFileSync(join(directory, "protocol.json"), "utf8"));
assert.equal(protocol.schema, 1);
assert.equal(protocol.experiment, "R092 immutable-artifact WANIX browser guard");
...
if (problems.length) throw new Error(`R092_WANIX_BROWSER_GATE_FAIL: ${problems.join("; ")}`);
console.log("R092_WANIX_BROWSER_GATE_PASS");
