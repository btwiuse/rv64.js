...
export const R093_ASSETS = Object.freeze({
  page: {
    path: join(root, "tests/jit-modern-boot-r093-browser.html"),
    route: "/jit-modern-boot-r093-browser.html",
  },
  worker: {
    path: join(root, "tests/jit-modern-boot-r093-browser-worker.mjs"),
    route: "/jit-modern-boot-r093-browser-worker.mjs",
  },
  timingLibrary: {
  ...
  wasmCandidate: {
    path: join(root, "target/bench/wasm-candidates/r093-product-b40bb5f3f55b.wasm"),
    route: "/rv64-candidate.wasm",
    expectedSha256: "b40bb5f3f55b04f7b6bea424011a7963313b363b3a8b0f8cbff161319bd3451e",
  },
  kernel: {
...
export function r093AssetManifest() {
  return Object.fromEntries(Object.entries(R093_ASSETS).map(([name, asset]) => {
...
export async function startR093Server() {
  const byRoute = new Map(Object.values(R093_ASSETS).map((asset) => [asset.route, asset]));
...
async function pageTarget(browserUrl, expected) {
  ...
  throw new Error("R093 benchmark page target did not appear");
}
...
async function collect(cdp) {
  ...
  throw new Error(`R093 browser benchmark timeout: ${lastError ?? "no result"}`);
}

export async function runR093BrowserSample({ origin, variant, chrome }) {
  if (variant !== "control" && variant !== "candidate") {
    throw new Error(`invalid R093 variant ${variant}`);
  }
  const profile = mkdtempSync(join(tmpdir(), `rv64-r093-${variant}-chrome-`));
  const url = `${origin}/jit-modern-boot-r093-browser.html?variant=${variant}`;
