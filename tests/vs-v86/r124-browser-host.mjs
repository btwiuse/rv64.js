...
export const R124_ASSETS = Object.freeze({
  page: {
    path: join(root, "tests/jit-modern-boot-r124-browser.html"),
    route: "/jit-modern-boot-r124-browser.html",
  },
  worker: {
    path: join(root, "tests/jit-modern-boot-r124-browser-worker.mjs"),
    route: "/jit-modern-boot-r124-browser-worker.mjs",
  },
  timingLibrary: {
  ...
  wasmControl: {
    path: join(root, "target/bench/r124-rvc-bank-hybrid/product-preflight/artifacts/control-d9f686a9.wasm"),
    route: "/rv64-control.wasm",
    expectedSha256: "d9f686a9ce4f54734f310cf5789f900af0c9b7067539e0251c429f376dd3df6d",
  },
  wasmCandidate: {
    path: join(root, "target/bench/r124-rvc-bank-hybrid/product-preflight/artifacts/candidate-d017a10f.wasm"),
    route: "/rv64-candidate.wasm",
    expectedSha256: "d017a10f00a8813df7c1af5750c8d4cb3a74b0dccea548eea31850a4ac56da59",
  },
  kernel: {
...
export function r124AssetManifest() {
  return Object.fromEntries(Object.entries(R124_ASSETS).map(([name, asset]) => {
...
export async function startR124Server() {
  const byRoute = new Map(Object.values(R124_ASSETS).map((asset) => [asset.route, asset]));
...
async function pageTarget(browserUrl, expected) {
  ...
  throw new Error("R124 benchmark page target did not appear");
}
...
async function collect(cdp) {
  ...
  throw new Error(`R124 browser benchmark timeout: ${lastError ?? "no result"}`);
}

export async function runR124BrowserSample({ origin, variant, chrome }) {
  if (variant !== "control" && variant !== "candidate") {
    throw new Error(`invalid R124 variant ${variant}`);
  }
  const profile = mkdtempSync(join(tmpdir(), `rv64-r124-${variant}-chrome-`));
  const url = `${origin}/jit-modern-boot-r124-browser.html?variant=${variant}`;
