...
// R085's integration guard. Every leg gets a fresh Chrome process, profile,
...
if (process.argv.length !== 3) {
  throw new Error("usage: node tests/run-r085-wanix-browser-pairs.mjs RESULTS_DIR");
}
...
assert.equal(hostCpuAffinity, "8-15", "R085 formal runner must be pinned to CPUs 8-15");

const lockPath = resolve(tmpdir(), "rv64-r085-wanix-browser.lock");
...
for (let attempt = 0; attempt < 3; attempt++) {
  try {
    ...
    if (Number.isSafeInteger(owner) && owner > 0 && processIsAlive(owner)) {
      throw new Error(`another R085 WANIX benchmark is running (pid ${owner})`);
    }
    rmSync(lockPath, { recursive: true, force: true });
    if (attempt === 2) throw new Error("could not acquire the R085 WANIX lock");
  }
}
let lockHeld = true;
...
const pages = {
  control: {
    url: "http://127.0.0.1:8765/examples/" +
      "v86-rv64-three-way-r085-control-414a174542161f9d.html",
    path: resolve(root,
      "integrations/wanix/v86-rv64-three-way-r085-control-414a174542161f9d.html"),
    sha256: "bf1b4dbb6553da7d88fb1991993087e35e52b3fd2e5fc369ba858bb2a78adc4d",
  },
  candidate: {
    url: "http://127.0.0.1:8765/examples/" +
      "v86-rv64-three-way-r085-candidate-0b953be67610e130.html",
    path: resolve(root,
...
const archives = {
  control: {
    path: resolve(root, "target/bench/r085-fast-jit-state-hash/browser-assets/" +
      "rv64-jit-r085-control-414a174542161f9d.tgz"),
    url: "http://127.0.0.1:8765/rv64/rv64-jit-r085-control-414a174542161f9d.tgz",
    sha256: "414a174542161f9d52d6814d1deaf9fbdd56e4fa152d11fa80d7167e76a45aa5",
    wasmSha256: "e5415db83b27b32a1f525af2aa19e93539332a274068e389a1e28ebba41d8095",
  },
  candidate: {
    path: resolve(root, "target/bench/r085-fast-jit-state-hash/browser-assets/" +
...
const protocol = {
  schema: 1,
  experiment: "R085 immutable-artifact WANIX browser guard",
  ...
  toolManifest: {
    ...
    analyzer: digestFile(resolve(root, "tests/analyze-r085-wanix-browser-pairs.mjs")),
...
try {
  for (const { pair, sides } of order) {
    for (const side of sides) {
      process.stderr.write(`R085 WANIX pair ${pair}/${pairs}: ${side}\n`);
...
process.stderr.write(`R085 fixed WANIX sample complete: ${resultsDirectory}\n`);
