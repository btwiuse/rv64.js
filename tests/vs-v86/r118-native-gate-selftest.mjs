...
const gate = join(here, "r118-native-gate.mjs");
const temp = await mkdtemp(join(tmpdir(), "r118-native-gate-"));
const controlWasm = "d9f686a9ce4f54734f310cf5789f900af0c9b7067539e0251c429f376dd3df6d";
const candidateWasm = "0501207f314fd134ba981d41016d3c16ae8de70ce4278aca24b8fb22e5d6bd92";
const loader = "2cbb264f4dac9d32e96254a4b12961c84a862e2bf3b6a5adcba248d2ad7b4385";
const rows = ["boot", "compile", "python"];

function makeNative({ bootCandidate = 980, compileCandidate = 1000, pythonCandidate = 1000 } = {}) {
...
try {
  ...
  assert.equal((await evaluate("sub-one-percent", makeNative({ bootCandidate: 995 })))
    .admitProductGates, false);
  assert.equal((await evaluate("protected-regression", makeNative({ pythonCandidate: 1015 })))
    .admitProductGates, false);
  console.log("R118 native gate selftest: verified target and protected-row decisions");
} finally {
