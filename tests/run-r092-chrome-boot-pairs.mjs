...
import {
  browserExecutable, r092AssetManifest, runR092BrowserSample, startR092Server,
} from "./vs-v86/r092-browser-host.mjs";

if (process.argv.length !== 3) {
  throw new Error("usage: node tests/run-r092-chrome-boot-pairs.mjs RESULTS_DIR");
}
...
assert.equal(hostCpuAffinity, "8-15", "R092 formal runner must be pinned to CPUs 8-15");

const lockPath = resolve(tmpdir(), "rv64-r092-chrome-boot.lock");
...
for (let attempt = 0; attempt < 3; attempt++) {
  try {
    ...
    if (Number.isSafeInteger(owner) && owner > 0 && processIsAlive(owner)) {
      throw new Error(`another R092 browser Boot benchmark is running (pid ${owner})`);
    }
    rmSync(lockPath, { recursive: true, force: true });
    if (attempt === 2) throw new Error("could not acquire the R092 browser Boot lock");
  }
}
let lockHeld = true;
...
const protocol = {
  schema: 1,
  experiment: "R092 artifact A/B Chrome execution-only modern Boot",
  ...
  configurations: {
    ...
    candidate: { wasm: "R092", policy: "production-page" },
  },
  assetManifest: r092AssetManifest(),
  toolManifest: {
    host: digestFile(resolve(root, "tests/vs-v86/r092-browser-host.mjs")),
    runner: digestFile(fileURLToPath(import.meta.url)),
    analyzer: digestFile(resolve(root, "tests/analyze-r092-chrome-boot-pairs.mjs")),
    selftest: digestFile(resolve(root, "tests/vs-v86/r092-harness-selftest.mjs")),
    worker: digestFile(resolve(root, "tests/jit-modern-boot-r092-browser-worker.mjs")),
...
const server = await startR092Server();
try {
  for (const { pair, sides } of order) {
    for (const side of sides) {
      process.stderr.write(`R092 Chrome Boot pair ${pair}/${pairs}: ${side}\n`);
      const sample = await runR092BrowserSample({
...
process.stderr.write(`R092 fixed Chrome Boot sample complete: ${resultsDirectory}\n`);
