...
//! feature. This rv64.js-vendored build gets time from the embedding runtime's
//! raw `host_unix_ms` WebAssembly import. This replaces upstream's wasm-bindgen
//! bridge while preserving the crate's public API.
...
impl UnixTime {
    ...
    pub fn now() -> Self {
        ...
        {
            // The embedding host supplies Date.now() through rv64.js's plain
            // `env` import table. Rust's float-to-int cast saturates invalid or
            // pre-epoch values to zero.
            let unix_ms = unsafe { host_unix_ms() } as u64;
            return Self::since_unix_epoch(Duration::from_millis(unix_ms));
        }
