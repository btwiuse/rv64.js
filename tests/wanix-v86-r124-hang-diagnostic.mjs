...
// Diagnostic-only clone of the frozen WANIX harness. It emits visible phase
// release tokens and periodic RV64/P9 progress snapshots to distinguish a lost
// console release from a stalled external 9P exchange. Its timings are never
...
async function waitForBenchmarkCount(
  cdp, phase, count, timeoutMs, jitWorker = null, phaseStart = null,
) {
  const deadline = Date.now() + timeoutMs;
  const started = Date.now();
  let nextDiagnostic = started;
  const definition = phaseDefinitions[phase];
  let terminal = "";
  while (Date.now() < deadline) {
    terminal = await evaluate(cdp, terminalExpression);
    ...
    if (jitWorker && phaseStart && Date.now() >= nextDiagnostic) {
      const stats = await readJitStats(cdp, jitWorker);
      const delta = jitDelta(phaseStart, stats);
      console.error(`__R124_HANG_DIAGNOSTIC__${JSON.stringify({
        phase,
        count,
        elapsedMs: Date.now() - started,
        instructions: delta.instructions,
        generatedRetired: delta.generatedRetired,
        interpreterRetired: delta.interpreterRetired,
        p9Requests: delta.p9Requests,
        p9Pending: stats.p9.pending,
        p9Outstanding: stats.p9.outstanding ?? null,
        p9MaximumPending: delta.p9MaximumPending,
        p9ReadRequests: delta.p9ReadRequests,
        p9WriteRequests: delta.p9WriteRequests,
        p9ReadBytes: delta.p9ReadBytes,
        p9WriteBytes: delta.p9WriteBytes,
        p9HostMs: delta.p9HostMs,
        modules: delta.modules,
      })}`);
      nextDiagnostic = Date.now() + 5_000;
    }
    await delay(250);
  }
  ...
  for (let repetition = 0; repetition < benchRepetitions; repetition++) {
    ...
    for (const phase of requestedPhases) {
      ...
      if (phaseSync) {
        ...
        const releaseToken = `R124_RELEASE_${phase}_${repetition + 1}`;
        await send(pageCdp, releaseToken);
        terminal = await waitForTextCount(pageCdp, releaseToken, 1, 30_000);
        console.error(`__R124_RELEASE_OBSERVED__${releaseToken}`);
      }
      try {
        terminal = await waitForBenchmarkCount(
          pageCdp, phase, repetition + 1, benchTimeoutMs,
          jitWorker, jitSnapshots?.[startKey] ?? null,
        );
      } catch (error) {
        if (jitWorker) {
          const stats = await readJitStats(pageCdp, jitWorker);
          const delta = jitDelta(jitSnapshots[startKey], stats);
          error.message += `\nRV64 diagnostic: ${JSON.stringify({
            instructions: delta.instructions,
            generatedRetired: delta.generatedRetired,
            interpreterRetired: delta.interpreterRetired,
            p9Requests: delta.p9Requests,
