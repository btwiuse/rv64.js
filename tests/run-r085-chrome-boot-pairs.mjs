...
import {
  browserExecutable, r085AssetManifest, runR085BrowserSample, startR085Server,
} from "./vs-v86/r085-browser-host.mjs";

if (process.argv.length !== 3) {
  throw new Error("usage: node tests/run-r085-chrome-boot-pairs.mjs RESULTS_DIR");
}
...
const lockPath = resolve(tmpdir(), "rv64-r085-chrome-boot.lock");
...
for (let attempt = 0; attempt < 3; attempt++) {
  try {
    ...
    if (Number.isSafeInteger(owner) && owner > 0 && processIsAlive(owner)) {
      throw new Error(`another R085 browser Boot benchmark is running (pid ${owner})`);
    }
    rmSync(lockPath, { recursive: true, force: true });
    if (attempt === 2) throw new Error("could not acquire the R085 browser Boot lock");
  }
}
let lockHeld = true;
...
const protocol = {
  ...
  expectedBrowser,
  configurations: {
    control: { wasm: "R080", policy: "production-page" },
    candidate: { wasm: "R085", policy: "production-page" },
  },
  assetManifest: r085AssetManifest(),
  toolManifest: {
    host: digestFile(resolve(root, "tests/vs-v86/r085-browser-host.mjs")),
    runner: digestFile(fileURLToPath(import.meta.url)),
    analyzer: digestFile(resolve(root, "tests/analyze-r085-chrome-boot-pairs.mjs")),
    selftest: digestFile(resolve(root, "tests/vs-v86/r085-harness-selftest.mjs")),
    worker: digestFile(resolve(root, "tests/jit-modern-boot-r085-browser-worker.mjs")),
...
const server = await startR085Server();
try {
  for (const { pair, sides } of order) {
    for (const side of sides) {
      process.stderr.write(`R085 Chrome Boot pair ${pair}/${pairs}: ${side}\n`);
      const sample = await runR085BrowserSample({
...
process.stderr.write(`R085 fixed Chrome Boot sample complete: ${resultsDirectory}\n`);
