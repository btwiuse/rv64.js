...
export const R085_ASSETS = Object.freeze({
  page: {
    path: join(root, "tests/jit-modern-boot-r085-browser.html"),
    route: "/jit-modern-boot-r085-browser.html",
  },
  worker: {
    path: join(root, "tests/jit-modern-boot-r085-browser-worker.mjs"),
    route: "/jit-modern-boot-r085-browser-worker.mjs",
  },
  timingLibrary: {
  ...
  wasmControl: {
    path: join(root, "target/bench/wasm-candidates/r080-residual-clean-e5415db83b27.wasm"),
    route: "/rv64-control.wasm",
    expectedSha256: "e5415db83b27b32a1f525af2aa19e93539332a274068e389a1e28ebba41d8095",
  },
  wasmCandidate: {
...
export function r085AssetManifest() {
  return Object.fromEntries(Object.entries(R085_ASSETS).map(([name, asset]) => {
...
export async function startR085Server() {
  const byRoute = new Map(Object.values(R085_ASSETS).map((asset) => [asset.route, asset]));
...
async function pageTarget(browserUrl, expected) {
  ...
  throw new Error("R085 benchmark page target did not appear");
}
...
async function collect(cdp) {
  ...
  throw new Error(`R085 browser benchmark timeout: ${lastError ?? "no result"}`);
}

export async function runR085BrowserSample({ origin, variant, chrome }) {
  if (variant !== "control" && variant !== "candidate") {
    throw new Error(`invalid R085 variant ${variant}`);
  }
  const profile = mkdtempSync(join(tmpdir(), `rv64-r085-${variant}-chrome-`));
  const url = `${origin}/jit-modern-boot-r085-browser.html?variant=${variant}`;
