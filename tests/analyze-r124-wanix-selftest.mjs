...
// Synthetic decision tests for the frozen R124 WANIX analyzer. No guest or
...
const analyzer = resolve(root, "tests/analyze-r124-wanix-browser-pairs.mjs");
...
function makeFixture(mutate = () => {}) {
  const directory = mkdtempSync(join(tmpdir(), "r124-wanix-analyzer-"));
  const protocol = {
    schema: 1,
...
try {
  ...
  assert.match(validResult.stdout, /R124_WANIX_BROWSER_GATE_PASS/);
...
console.log("R124 WANIX analyzer selftest: pass, confidence regression, P9 bytes, active JIT, and spread guards enforced");
