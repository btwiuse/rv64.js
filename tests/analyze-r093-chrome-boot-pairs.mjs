...
if (!process.argv[2] || !outputArgument) {
  throw new Error(
    "usage: node tests/analyze-r093-chrome-boot-pairs.mjs RESULTS_DIR --output=REPORT",
  );
}
...
assert.equal(protocol.experiment, "R093 artifact A/B Chrome execution-only modern Boot");
...
const expectedWasm = {
  ...
  candidate: "b40bb5f3f55b04f7b6bea424011a7963313b363b3a8b0f8cbff161319bd3451e",
};
const runs = [];
...
console.log(
  `R093 Chrome Boot: ${pairedMedian.toFixed(3)}x ` +
  `CI=[${confidence95.map((value) => value.toFixed(3)).join(", ")}]`,
);
if (problems.length) throw new Error(`R093_CHROME_BOOT_GATE_FAIL: ${problems.join("; ")}`);
console.log("R093_CHROME_BOOT_GATE_PASS");
