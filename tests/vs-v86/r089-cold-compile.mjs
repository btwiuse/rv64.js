...
// Seven alternating fresh-process cold compile/instantiate pairs for R089.
...
if (!controlPath || !candidatePath || !outputPath) {
  throw new Error("usage: r089-cold-compile.mjs CONTROL.wasm CANDIDATE.wasm OUTPUT.json");
}
...
if (control.sha256 !==
  ...
  problems.push("control is not exact R085");
}
if (control.imports !== candidate.imports || control.exports !== candidate.exports) {
  problems.push("candidate import/export count changed");
}
...
console.log(`control compile ${median(runs.control.map((run) => run.compileMs)).toFixed(3)} ms`);
console.log(`candidate compile ${median(runs.candidate.map((run) => run.compileMs)).toFixed(3)} ms`);
