...
const VMEM = (load, base, reg) =>
  ((load ? 0x07 : 0x27) | (reg << 7) | (base << 15) | (1 << 25)) >>> 0;
...
function program() {
  ...
  code.push(
    ...
    VMEM(false, 21, 8),         // vse8.v v8,(x21)
    VMEM(true, 20, 9),          // vle8.v v9,(x20)
...
console.log(
  ...
  `blocks=${jit.jitBlocks} vector-modules=${jit.vectorModules}`,
);
