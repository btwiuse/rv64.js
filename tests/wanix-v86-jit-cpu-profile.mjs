...
const cpuProfilePrefix = process.env.WANIX_CPU_PROFILE_PREFIX;
const bootCpuProfile = process.env.WANIX_CPU_PROFILE_BOOT === "1";
...
if (cpuProfilePrefix && (vmId !== "rv64-jit" || !phaseSync)) {
  throw new Error("WANIX_CPU_PROFILE_PREFIX requires rv64-jit and synchronized phases");
}
if (bootCpuProfile && !cpuProfilePrefix) {
  throw new Error("WANIX_CPU_PROFILE_BOOT requires WANIX_CPU_PROFILE_PREFIX");
}
...
try {
  ...
  let jitWorker = null;
  if (bootCpuProfile) {
    // The adapter exposes the VM immediately before vm.start. Attaching here
    // captures essentially the complete firmware/Linux execution while
    // keeping profiler overhead outside every scored R075 result.
    jitWorker = await findBenchmarkWorker(pageCdp);
    await pageCdp.send("Profiler.enable", {}, jitWorker);
    await pageCdp.send("Profiler.setSamplingInterval", { interval: 100 }, jitWorker);
    await pageCdp.send("Profiler.start", {}, jitWorker);
  }
  await waitForText(pageCdp, "WANIX shell ready", bootTimeoutMs);
  const shellMs = performance.now() - bootStart;
  let jitAtShell = null;
  if (bootCpuProfile) {
    const { profile: bootProfile } = await pageCdp.send("Profiler.stop", {}, jitWorker);
    writeFileSync(
      `${cpuProfilePrefix}-boot.cpuprofile`,
      `${JSON.stringify(bootProfile)}\n`,
      { flag: "wx" },
    );
    jitAtShell = await readJitStats(pageCdp, jitWorker);
  }
  ...
  jitWorker ??= vmId === "rv64-jit" ? await findBenchmarkWorker(pageCdp) : null;
  ...
  if (
    jitWorker && !prebootJitConfiguration &&
  ...
  if (prebootJitConfiguration && jitWorker) {
    jitAtShell ??= await readJitStats(pageCdp, jitWorker);
    const expectedSampled = jitConfiguration.sampledStaticT0 === true;
    const expectedBackoff = jitConfiguration.sampledStaticT0Backoff === true;
    assert.equal(jitAtShell.staticT0.systemEnabled, false, "preboot residual static-T0");
    assert.equal(jitAtShell.staticT0.sampledEnabled, expectedSampled, "preboot sampled static-T0");
    assert.equal(
      jitAtShell.staticT0.sampledBackoffEnabled,
      expectedBackoff,
      "preboot sampled-backoff static-T0",
    );
    assert.equal(BigInt(jitAtShell.staticT0.systemErrors), 0n, "preboot static-T0 errors");
  }
  if (cpuProfilePrefix && !bootCpuProfile) {
    await pageCdp.send("Profiler.enable", {}, jitWorker);
    await pageCdp.send("Profiler.setSamplingInterval", { interval: 100 }, jitWorker);
  }
  const jitSnapshots = jitWorker ? { before: await readJitStats(pageCdp, jitWorker) } : null;
  ...
  for (let repetition = 0; repetition < benchRepetitions; repetition++) {
    ...
    for (const phase of requestedPhases) {
      ...
      if (cpuProfilePrefix) {
        const { profile } = await pageCdp.send("Profiler.stop", {}, jitWorker);
        const profileKey = `${phase}${repetition + 1}`;
        writeFileSync(
          `${cpuProfilePrefix}-${profileKey}.cpuprofile`,
  ...
  if (jitSnapshots) {
    ...
    console.log(JSON.stringify(summaryOnly
      ? { ...(jitAtShell ? { jitAtShell } : {}), jitPhases }
      : { ...(jitAtShell ? { jitAtShell } : {}), jitSnapshots, jitPhases }));
    if (jitProfile) {
      console.log(JSON.stringify({
