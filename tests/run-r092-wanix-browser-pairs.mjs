...
// R092's integration guard. Every leg gets a fresh Chrome process, profile,
...
if (process.argv.length !== 3) {
  throw new Error("usage: node tests/run-r092-wanix-browser-pairs.mjs RESULTS_DIR");
}
...
const lockPath = resolve(tmpdir(), "rv64-r092-wanix-browser.lock");
...
for (let attempt = 0; attempt < 3; attempt++) {
  try {
    ...
    if (Number.isSafeInteger(owner) && owner > 0 && processIsAlive(owner)) {
      throw new Error(`another R092 WANIX benchmark is running (pid ${owner})`);
    }
    rmSync(lockPath, { recursive: true, force: true });
    if (attempt === 2) throw new Error("could not acquire the R092 WANIX lock");
  }
}
let lockHeld = true;
...
const pages = {
  ...
  candidate: {
    url: "http://127.0.0.1:8765/examples/" +
      "v86-rv64-three-way-r092-candidate-e0488c48b3ea3980.html",
    path: resolve(root,
...
const archives = {
  ...
  candidate: {
    path: resolve(root, "target/bench/r092-member-range/browser-assets/" +
      "rv64-jit-r092-member-range-e0488c48b3ea3980.tgz"),
    url: "http://127.0.0.1:8765/rv64/rv64-jit-r092-member-range-e0488c48b3ea3980.tgz",
    sha256: "e0488c48b3ea398060a42854715883eea4b34c96a13d397c5068f8a8e1c4029b",
    wasmSha256: "5baeccb5c5feaf2d3f7605fd42f741f9cbaa89e566a86c0bbea201a3c6389023",
  },
};
...
const protocol = {
  schema: 1,
  experiment: "R092 immutable-artifact WANIX browser guard",
  ...
  toolManifest: {
    ...
    analyzer: digestFile(resolve(root, "tests/analyze-r092-wanix-browser-pairs.mjs")),
...
try {
  for (const { pair, sides } of order) {
    for (const side of sides) {
      process.stderr.write(`R092 WANIX pair ${pair}/${pairs}: ${side}\n`);
...
process.stderr.write(`R092 fixed WANIX sample complete: ${resultsDirectory}\n`);
