...
// docs/jit-rewrite/R072_SAMPLED_STATIC_T0_PROTOCOL.md. Keep this verifier
// candidate-specific: changing any constant creates a new protocol, and an
// existing report must continue to be judged by these exact rules.
...
if (!reportArg) {
  throw new Error("usage: node tests/vs-v86/r072-sampled-static-gate.mjs REPORT.json");
}
...
const EXPECTED_CANDIDATE = {
  SCORECARD_V2_STATIC_SYSTEM_T0: "1",
...
if (!same(report.configuration?.rows, EXPECTED_ROWS)) {
  problems.push(
    `rows ${JSON.stringify(report.configuration?.rows)}, expected ${JSON.stringify(EXPECTED_ROWS)}`,
  );
}
...
for (const trial of report.trials ?? []) {
  ...
  if (
    ...
    lifecycle.sampled !== shouldEnable ||
    lifecycle.modulesAfter !== lifecycle.modulesBefore + 1
  ) {
    problems.push(`${trial.side}/${trial.row}/rep${trial.rep}: invalid sampled-static lifecycle`);
  }
  ...
  if (shouldEnable && (fast === 0n || sampled === 0n || samples === 0n || polls === 0n)) {
    problems.push(
      `${trial.side}/${trial.row}/rep${trial.rep}: incomplete sampled-static proof ` +
        `(fast=${fast}, sampled=${sampled}, samples=${samples}, polls=${polls})`,
    );
  }
  if (!shouldEnable && (fast !== 0n || sampled !== 0n || samples !== 0n || polls !== 0n)) {
    problems.push(
      `${trial.side}/${trial.row}/rep${trial.rep}: control executed sampled static T0`,
    );
  }
  if (errors !== 0n) {
...
if (problems.length) {
  console.error(`R072_GATE_B_FAIL: ${problems.join("; ")}`);
  process.exitCode = 1;
} else {
  console.log("R072_GATE_B_PASS");
}
