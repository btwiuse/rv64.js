...
import { runTimedBoot } from "/r077-browser-boot-lib.mjs";
...
async function main() {
  ...
  if (productionLifecycle.registeredModules !== 1) {
    throw new Error(
      `production helper registered ${productionLifecycle.registeredModules} modules, expected one`,
    );
  }
  if (variant === "control") {
    // Same artifact and untimed helper lifecycle as production; disable only
    // execution so this leg reproduces R054-equivalent behavior.
    vm.ex.jit_set_static_t0_sampled_backoff(0);
    vm.ex.jit_set_static_t0_sampled(0);
  }
  ...
  return {
    ...
    staticLifecycle: {
      productionHelper: productionLifecycle.production,
      registeredModules: productionLifecycle.registeredModules,
      moduleIndex: productionLifecycle.index,
      modulesBefore: productionLifecycle.modulesBefore,
      modulesAfter: productionLifecycle.modulesAfter,
      systemEnabled: false,
      sampledEnabled: production,
      sampledBackoffEnabled: production,
      controlOverride: variant === "control",
    },
