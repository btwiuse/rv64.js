...
if (process.argv.length !== 3) {
  throw new Error("usage: node tests/run-r077-chrome-boot-pairs.mjs RESULTS_DIR");
}
...
assert.equal(hostCpuAffinity, "8-15", "R077 formal runner must be pinned to CPUs 8-15");

const lockPath = resolve(tmpdir(), "rv64-r077-chrome-boot.lock");
...
for (let attempt = 0; attempt < 3; attempt++) {
  try {
    ...
    if (Number.isSafeInteger(owner) && owner > 0 && processIsAlive(owner)) {
      throw new Error(`another R077 benchmark is running (pid ${owner})`);
    }
    rmSync(lockPath, { recursive: true, force: true });
    if (attempt === 2) throw new Error("could not acquire the R077 benchmark lock");
  }
}
let lockHeld = true;
...
const assetManifest = r077AssetManifest();
const toolManifest = {
  host: digestFile(resolve(root, "tests/vs-v86/r077-browser-host.mjs")),
  selftest: digestFile(resolve(root, "tests/vs-v86/r077-harness-selftest.mjs")),
...
try {
  for (const { pair, sides } of order) {
    for (const side of sides) {
      process.stderr.write(`R077 Chrome Boot pair ${pair}/${pairs}: ${side}\n`);
...
process.stderr.write(`R077 fixed Chrome Boot sample complete: ${resultsDirectory}\n`);
